...
{ 
  ...
  try{
	  ...
	  //here we define a set of keywords for a topic
	  String []topics={"Muhammad Ali"};
	  //adding more filter
	 
	  String []topic1 ={"boxing","champion"};
	  System.out.println("Key words: " + Arrays.toString(topics) + " and " + Arrays.toString(topic1));
	  for (int i = 0; i < maxPages; i++) 
	  	{
		  	...
		  	if (robotSafe(url)) 
		  	{
		  		...
		  		// now we need to check whether a page contains any of the keyword or not. If so, we will process the page and save all 
		  		//URLs inside the page to the frontier. Otherwise, we will just discard the current page (without saving any URLs inside the page
		  		// to the frontier.
		  		if(page.length()!=0)
		  		{
		  			for(int k=0;k<topics.length;k++)
		  			{
		  				...
		  				for(int t=0;t<topic1.length;t++)
	  					{
		  					if(page.toLowerCase().contains(topics[k].toLowerCase())& page.toLowerCase().contains(topic1[t].toLowerCase()))
		  						{
...
{ WebCrawlerTopics wc = new WebCrawlerTopics();
  wc.run(argv);
}
